...
Scheduler::Scheduler()
{               
    m_u8OpenSlots = static_cast<uint8_t>(NUMBER_OF_SLOTS); //PREGUNTAR
    m_u8NextSlot = 0;
...
uint8_t Scheduler::attach(Task * i_ToAttach, uint64_t i_u64TickInterval)
{
    ...
    l_st_StructToAttach.u64ticks = this->m_u64ticks; // Tick count
    l_st_StructToAttach.u64TickInterval = 0;
	l_st_StructToAttach.u64TickIntervalInitValue = i_u64TickInterval; //PREGUNTAR
...
uint8_t Scheduler::setup(void)
{
    ...
    while(l_iNextTaskSlot < NUMBER_OF_SLOTS)
    {
        l_pNextTask = static_cast<Task *> (m_aSchedule[l_iNextTaskSlot].pToAttach); //PREGUNTAR
        if(l_pNextTask != ((uintptr_t) 0))
        {
            l_pNextTask->setup();
        }
...
uint8_t Scheduler::CalculateNextSchedule(void)  // TODO
{
    return(NO_ERR);
}
uint8_t Scheduler::SortScheduleByPriority(Task * i_pSchedule)   // TODO
{
    return(NO_ERR);
}
